...
function Category({props}) {
    ...
    const {currentCategory} = useSelector(state=>state.servers)
    ...
    return  <div className="discover-col1">
        {
            props.map((item, key)=>{
                ...
                return  <button onClick={()=>currentCategory!==item.id?
                    dispatch(getServerByCategory({typeId:item.id})):null} key={key} className="discover-category-button px-2">

                    <svg aria-hidden="false" width="20" height="20" viewBox="0 0 24 24">
                        <g fill="none" fillRule="discover-evenodd">
