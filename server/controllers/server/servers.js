        ...
        if (userId){
            ...
            servers= servers.map(item=>{
                if (serversOfUser.find(serverUser=>serverUser.dataValues.userId===userId && serverUser.dataValues.serverId===item.id) ){
                    return {id:item.id,name:item.name,description:item.description,icon:item.icon, preview:item.preview, users:item.users.length, currentUser:true  }
                }
                else {
...
const discover = async (req, res) => {
    ...
    try {
        ...
        if (userId){
            ...
            servers= rows.map(item=>{
                return {id:item.id,name:item.name,description:item.description,icon:item.icon, preview:item.preview, users:item.users.length }
            })
        }
